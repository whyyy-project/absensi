...
$qry_siswa = mysqli_query($db, "SELECT * FROM `tb_siswa` LEFT JOIN tb_kelas ON tb_siswa.id_kelas = tb_kelas.id_kelas LEFT JOIN kariyawan ON kariyawan.id_kelas = tb_kelas.id_kelas WHERE kariyawan.id_kariyawan = $idKariyawan AND tb_siswa.status = 1;");
